...
###################################################################
        conv11 = self.conv1(FNC2)
        conv11 = self.activation(conv11)
        conv11 = self.norm1(conv11)

        conv22 = self.conv2(conv11)
        conv22 = self.activation(conv22)
        if self.instance_norm2 == True:
            conv22 = conv22.transpose(1, 2)
            conv22 = self.norm22(conv22)
        conv22 = self.flatten(conv22)
        # print(conv2.shape)
        # 全连接层1
        hidden11 = self.linear1(conv22)
        hidden11 = self.activation(hidden11)
        hidden1_drop1 = self.dropout1(hidden11)

        X = torch.cat((hidden1_drop, hidden1_drop1), dim=1)



        # 全连接层2
        hidden2 = self.linear2(X)
